...
const Button: React.FC<ButtonProps> = ({
  ...
  const buttonClasses = useMemo(() => {
    ...
    const responsiveClasses = responsive
      ? 'sm:text-left sm:flex-nowrap sm:space-x-2'
      : '';

    const widthClass = fullWidth ? 'w-full' : '';

    // Special handling for very small screens
    const mobileClasses = responsive
      ? 'text-xs sm:text-sm md:text-base'
      : '';

    return `${baseClasses} ${variantClasses[variant]} ${sizeClasses[size]} ${widthClass} ${responsiveClasses} ${mobileClasses} ${className}`;
  }, [variant, size, fullWidth, responsive, className]);

  // Используем либо leftIcon, либо icon с позицией left
  ...
  // Используем icon только с позицией right
